...
範例用法：
  ...
  python nycu_crawler_multithreaded.py
    ...
    parser.add_argument(
        ...
        help=f'學年度 (預設: {DEFAULT_YEAR})'
    )
    parser.add_argument(
        '--semester',
        type=int,
        default=DEFAULT_SEMESTER,
        help=f'學期，1=上學期，2=下學期，X=暑期 (預設: {DEFAULT_SEMESTER})'
    )
    parser.add_argument(
        '--outline',
        action='store_true',
    ...
    # 驗證參數
    if args.year < 100 or args.year > 200:
        print("錯誤: 學年度必須介於 100 到 200 之間")
        sys.exit(1)

    if args.semester not in [1, 2]:
